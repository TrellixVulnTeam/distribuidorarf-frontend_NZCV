...
@Component({
  ...
  styleUrls: ['./detalle-proforma.component.scss'],
  providers: [
...
export class DetalleProformaComponent implements OnInit {  
  ...
  crearProforma(){
    ...
    this.profomaFormSubmit.estado = this.isEdit && this.proforma.fechaVencimiento < fechaHoy ? this.strings.label_estado_vencida : this.strings.label_estado_atiempo;
    if(this.isEdit){
      this.profomaFormSubmit.idProforma = this.proforma.idProforma;
    }
  ...
  actualizarProforma(){
    this.crearListaDetallesProforma(this.proforma.idProforma);    
    let cuentaLineasActualizadas = 0;    
    this.detallesProforma.forEach(element => {
      console.log(element);
      if(element.detalleProforma != undefined){
        this.detallesProformasService.updateFullPreInvoice(element).subscribe(
          res => {
            cuentaLineasActualizadas++;
            if(cuentaLineasActualizadas === this.detallesProforma.length){
              // Elimina las líneas de la proforma
              let totalLineasBorradas = this.detallesProformaBorrar.length;
              let cuentaLineasBorradas = 0;
              if(totalLineasBorradas > 0){
                this.detallesProformaBorrar.forEach(element => {
                  this.detallesProformasService.deleteFullPreInvoice(element).subscribe(
                    res => {
                      cuentaLineasBorradas++;
                      if(cuentaLineasBorradas === totalLineasBorradas){
                        this.proformasService.update(this.profomaFormSubmit.idProforma, this.profomaFormSubmit).subscribe(
                          res => {
                            this.proformaInsertada = res;
                            if(this.convertToInvoice){
                              this.createInvoice(this.proformaInsertada.idProforma, this.proformaForm.controls.codigoResponsable.value, this.proformaInsertada.total);
                            }else{
                              this.isLoading = false;
                              this.loader.close();
                              this.snack.open('Proforma actualizada correctamente', 'OK', { duration: 5000 });              
                              window.location.reload();                            
                            }                            
                          },
                          err => {
                            this.reintento(err, nombresMetodos.actualizarProforma);
                          }
                        );      
                      }
                    }
        ...
        console.log("Actualizando detalle" + element.detalleProforma);
      }else{
  ...
  separateProduct(idProduct: string){
      ...
      if(this.error.statusCode == 401){
        ...
        this.tokenService.login().subscribe(
          res => {
              ...
              }else if(metodo === nombresMetodos.actualizarProforma){
                this.actualizarProforma();
